...
export default function App() {
  ...
  const [promptA, setPromptA] = useState('## TASK\nYou are a helpful assistant. Summarize this input user text precisely: \n\n## INPUT TEXT\n{{text}}');
  const [promptB, setPromptB] = useState('## ROLE: Expert Copywriter\nYou are rewriting input copy for clarity and maximum engagement. Keep it clean:\n\n## RAW INPUT\n{{text}}');
  
  const [variable, setVariable] = useState<Variable>({ key: 'text', value: 'Vite is a blazing fast frontend build tool powering the next generation of web development.' });
  ...
  const runEvaluation = async () => {
    if (!apiKey) return alert('PromptDiff requires a valid Gemini API Key to function. Enter it in the settings bar.');
    if (!variable.value) return alert('Please enter a test value for your variable bench.');
    setLoading(true);
    ...
    const discoverModel = async () => {
      ...
      const supportedModels = models.filter((m: any) => {
        ...
        return actions.some((action) => action.toLowerCase().includes('generatecontent'));
      });

      const selected = supportedModels.find((m: any) => /gemini/i.test(m.name)) || supportedModels[0];
      if (!selected) {
        throw new Error('No available models support generateContent for this API key.');
      }
    ...
    const fetchLLM = async (promptText: string) => {
      const normalizeModel = (rawModel: string) => rawModel.startsWith('models/') ? rawModel : `models/${rawModel}`;
      ...
      let modelResource = modelName ? normalizeModel(modelName) : await discoverModel();
      let { response, data } = await tryRequest(modelResource);

      if (!response.ok) {
        const message = String(data.error?.message || response.statusText).toLowerCase();
      ...
      return data.candidates?.[0]?.content?.parts?.[0]?.text || 'Error: Generation failed.';
    };

    try {
      const [resA, resB] = await Promise.all([fetchLLM(finalA), fetchLLM(finalB)]);
      setOutputA(resA);
      setOutputB(resB);
    } catch (err) {
      console.error(err);
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-6 flex flex-col gap-4 backdrop-blur-lg">
          ...
          <div className="grid grid-cols-[160px,1fr] gap-4 items-center bg-slate-950 p-3 rounded-lg border border-slate-800">
            <div className="relative">
              ...
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 font-mono text-sm">{'}'}</span>
            </div>
            <input 
              type="text" 
        ...
        <div className="border-t border-slate-800 bg-slate-900 min-h-[35vh]">
          <div className="flex h-full">
            ...
            <div className="w-1/2 border-r border-slate-800 flex flex-col">
              <div className="px-6 py-2 border-b border-slate-800 text-xs font-medium text-slate-500 uppercase tracking-wider">Output A</div>
              <div className="flex-1 w-full bg-slate-950 p-6 text-base font-sans whitespace-pre-wrap leading-relaxed text-slate-200 selection:bg-slate-700">
                {outputA || <span className="text-slate-700 font-mono">Running...</span>}
              </div>
            </div>
            
            {/* Output Panel B (Highlights Diff vs Output A automatically if styled) */}
            <div className="w-1/2 flex flex-col bg-slate-950">
              <div className="px-6 py-2 border-b border-slate-800 text-xs font-medium text-slate-500 uppercase tracking-wider">Output B</div>
              <div className="flex-1 w-full bg-slate-950 p-6 text-base font-sans whitespace-pre-wrap leading-relaxed text-slate-200 selection:bg-slate-700">
                {outputB || <span className="text-slate-700 font-mono">Running...</span>}
              </div>
            </div>
